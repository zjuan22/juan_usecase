...
for i in range(0, 7):
    ...
    for p in range(0, entries):
       ...
       IP(dst='4.0.0.1',src='4.0.0.10')/
       GRE()/
       #IP(dst=ipdst[p] ,src=ipsrc[p])/
       IP(dst='192.169.0.10',src='10.0.0.10')/
       TCP(sport=20, dport=80)/Raw(RandString(size=pktsize[i])))
       
       if i==0: 
          #Create trace file
          FILE = "echo " + macsrc_h[p] + " " + '10.0.0.10 ' + '192.168.0.1 ' +'120'+" 1 >> " +create_dir+ "/trace_trPR_bng_ul" + str(entries) + "_single.txt"
