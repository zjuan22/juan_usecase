...
for i in range(0, 7):
    ...
    for p in range(0, entries):
        ...
        pkts.append(Ether(dst='aa:1b:eb:df:44:3d',src=macsrc[p])/IP(dst='192.169.0.1',src='192.169.0.10')/TCP(sport=80, dport=120)/Raw(RandString(size=pktsize[i])))
        ...
        if f == 0:
           ...
           FILE = "echo "+str(macsrc[p])+" 10.0.0.10 " +"4.0.0.10 " + "20 1 >> "+create_dir +"/trace_trPR_bng_dl_" + str(entries) + "_"+data_t +".txt"  
           os.system(FILE)
