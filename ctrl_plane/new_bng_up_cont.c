...
int read_config_from_file(char * filename) {
  ...
  while (fgets(line, sizeof(line), f)) {
    ...
    if (line[0] == 'S') { //SMAC
      if (7 == sscanf(line, "%c %x:%x:%x:%x:%x:%x", & dummy, & smac[0], & smac[1], & smac[2], & smac[3], & smac[4], & smac[5])) {
        fill_smac_table(smac);
      } else {
    ...
    } else if (line[0] == 'O') { //Decap process outer
      if (8 == sscanf(line, "%c %x:%x:%x:%x:%x:%x %d%c", & dummy, & smac[0], & smac[1], & smac[2], & smac[3], & smac[4], & smac[5])) {
        fill_decap(smac);
      } else {
        printf("Wrong format error in line\n");
        fclose(f);
